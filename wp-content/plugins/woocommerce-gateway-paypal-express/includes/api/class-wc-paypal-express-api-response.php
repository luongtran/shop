...
class WC_Paypal_Express_API_Response {
	...
	public function get_api_error_message() {
		...
		foreach ( range( 0, 9 ) as $index ) {
			...
			if ( $this->has_parameter( "L_SHORTMESSAGE{$index}" ) ) {
				...
				$error_message = sprintf( '%s: %s - %s',
					$this->has_parameter( "L_SEVERITYCODE{$index}" ) ? $this->get_parameter( "L_SEVERITYCODE{$index}" ) : __( 'Error', WC_PayPal_Express::TEXT_DOMAIN ),
					$this->has_parameter( "L_SHORTMESSAGE{$index}" ) ? $this->get_parameter( "L_SHORTMESSAGE{$index}" ) : __( 'Unknown', WC_PayPal_Express::TEXT_DOMAIN ),
					$this->has_parameter( "L_LONGMESSAGE{$index}" ) ? $this->get_parameter( "L_LONGMESSAGE{$index}" ) : __( 'Unknown error', WC_PayPal_Express::TEXT_DOMAIN )
				);
		...
		return empty( $error_messages ) ? __( 'N/A', WC_PayPal_Express::TEXT_DOMAIN ) : trim( implode( ', ', $error_messages ) );
	}
