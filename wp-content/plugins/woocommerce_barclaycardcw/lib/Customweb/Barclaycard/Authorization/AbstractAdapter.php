...
require_once 'Customweb/Barclaycard/Authorization/Hidden/AuthorizationParameterBuilder.php';


abstract class Customweb_Barclaycard_Authorization_AbstractAdapter extends Customweb_Barclaycard_AbstractAdapter {
	...
	protected function handleFailedTransaction(Customweb_Barclaycard_Authorization_Transaction $transaction, $failedTransaction) {
		if ($failedTransaction !== null && $failedTransaction instanceof Customweb_Barclaycard_Authorization_Transaction) {
	...
	public function authorize(Customweb_Payment_Authorization_ITransaction $transaction, array $parameters) {
		...
		// In all other cases we have to send the authorization request to the remote side
			...
			try {
				$builder = new Customweb_Barclaycard_Authorization_Hidden_AuthorizationParameterBuilder($transaction, $this->getContainer(), $parameters);
	...
	public function finalizeAuthorizationRequest(Customweb_Payment_Authorization_ITransaction $transaction) {
		...
		if ($transaction->isAuthorizationFailed()) {
			if (Customweb_Payment_Authorization_Moto_IAdapter::AUTHORIZATION_METHOD_NAME == $transaction->getAuthorizationMethod()) {
				return 'redirect:' . $transaction->getBackendFailedUrl();
			}
			else {
		...
		if ($transaction->isAuthorized()) {
			if (Customweb_Payment_Authorization_Moto_IAdapter::AUTHORIZATION_METHOD_NAME == $transaction->getAuthorizationMethod()) {
				return 'redirect:' . $transaction->getBackendSuccessUrl();
			}
			else {
