...
require_once 'Customweb/Barclaycard/Authorization/AbstractAdapter.php';
 ...
 *
 */
class Customweb_Barclaycard_Authorization_Moto_Adapter extends Customweb_Barclaycard_Authorization_AbstractAdapter implements Customweb_Payment_Authorization_Moto_IAdapter{
	...
	public function getFormActionUrl(Customweb_Payment_Authorization_ITransaction $transaction) {
		...
		if ($adapter instanceof Customweb_Barclaycard_Authorization_Hidden_Adapter) {
	...
	public function processAuthorization(Customweb_Payment_Authorization_ITransaction $transaction, array $parameters) {
		global $barclaycard_form_url, $barclaycard_form_parameters;
		...
		if ($adapter instanceof Customweb_Barclaycard_Authorization_Hidden_Adapter) {
			...
			if (isset($parameters['STATUS'])) {
				...
				$barclaycard_form_url = $adapter->getFormActionUrl($transaction, $parameters);
				$barclaycard_form_parameters = $adapter->getParameters($transaction, $parameters);
	...
	public function finalizeAuthorizationRequest(Customweb_Payment_Authorization_ITransaction $transaction) {
		...
		if ($adapter instanceof Customweb_Barclaycard_Authorization_Hidden_Adapter) {
			...
			if (isset($barclaycard_form_url) && !empty($barclaycard_form_url)) {
				$output = '<html><body>';
					$output .= '<form name="redirectionform" action="' . $barclaycard_form_url . '" method="POST">';
						foreach ($barclaycard_form_parameters as $key => $value) {
	...
	protected function getAdapterInstanceByPaymentMethod(Customweb_Payment_Authorization_IPaymentMethod $paymentMethod) {
		return new Customweb_Barclaycard_Authorization_Hidden_Adapter($this->getConfiguration()->getConfigurationAdapter(), $this->getContainer());
	}
