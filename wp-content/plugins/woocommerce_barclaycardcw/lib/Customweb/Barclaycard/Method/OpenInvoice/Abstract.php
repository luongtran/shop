 ...
 */
class Customweb_Barclaycard_Method_OpenInvoice_Abstract extends Customweb_Barclaycard_Method_DefaultMethod {
	...
	protected function getSalutationElements(Customweb_Payment_Authorization_IOrderContext $orderContext, $failedTransaction) {
		...
		if (empty($salutation)) {
			...
			$control->addValidator(new Customweb_Form_Validator_MaximalLength($control, Customweb_I18n_Translation::__("The salutation can not be longer than 5 chars."), 5));
	 ...
	 * @return DateTime
	 */
	protected function getBirthdate(Customweb_Barclaycard_Authorization_Transaction $transaction, array $formData) {
	...
	protected function getShippingBirthdateParameter(Customweb_Barclaycard_Authorization_Transaction $transaction, array $formData) {
		return array(
			'ECOM_SHIPTO_DOB' => $this->getBirthdate($transaction, $formData)->format('d/m/Y'),
		);
	}
	
	protected function getCustomerIdParameter(Customweb_Barclaycard_Authorization_Transaction $transaction) {
		...
		$id = Customweb_Barclaycard_Util::substrUtf8($id, 0, 50);
	...
	protected function getCustomerGender(Customweb_Barclaycard_Authorization_Transaction $transaction, array $formData) {
	...
	protected function getCustomerGenderParameter(Customweb_Barclaycard_Authorization_Transaction $transaction, array $formData) {
		return array(
			'ECOM_CONSUMER_GENDER' => $this->getCustomerGender($transaction, $formData),
		);
	}
	
	protected function getSalutation(Customweb_Barclaycard_Authorization_Transaction $transaction, array $formData) {
		$orderContext = $transaction->getTransactionContext()->getOrderContext();
		$salutation = $orderContext->getBillingSalutation();
		if (!empty($salutation)) {
	...
	protected function getBillingCivilityParameters(Customweb_Barclaycard_Authorization_Transaction $transaction, array $formData) {
		return array(
			'CIVILITY' => Customweb_Util_String::substrUtf8($this->getSalutation($transaction, $formData), 0, 10),
		);
	}
	
	protected function getSalesTaxNumber(Customweb_Barclaycard_Authorization_Transaction $transaction, array $formData) {
		$orderContext = $transaction->getTransactionContext()->getOrderContext();
	...
	protected function getSalesTaxNumberParameters(Customweb_Barclaycard_Authorization_Transaction $transaction, array $formData) {
	...
	protected function getPhoneNumber(Customweb_Barclaycard_Authorization_Transaction $transaction, array $formData) {
		$orderContext = $transaction->getTransactionContext()->getOrderContext();
	...
	protected function getOwnerPhoneNumberParameters(Customweb_Barclaycard_Authorization_Transaction $transaction, array $formData) {
		return array(
			'OWNERTELNO' => Customweb_Util_String::substrUtf8($this->getPhoneNumber($transaction, $formData), 0, 30),
		);
	}
	
	protected function getCustomerReferenceIdParameters(Customweb_Barclaycard_Authorization_Transaction $transaction, array $formData) {
	...
	protected function getCommericalRegisterNumber(Customweb_Barclaycard_Authorization_Transaction $transaction, array $formData) {
		$orderContext = $transaction->getTransactionContext()->getOrderContext();
	...
	protected function getSocialSecurityNumber(Customweb_Barclaycard_Authorization_Transaction $transaction, array $formData) {
		$orderContext = $transaction->getTransactionContext()->getOrderContext();
