...
/**
 * This util class some basic functions for Barclaycard.
 * 
 * @author Thomas Hunziker
 *
 */
final class Customweb_Barclaycard_Util {	
	
	private function __construct() {
		// prevent any instantiation of this class	
	}
	
	public static function applyOrderSchema(Customweb_Barclaycard_Configuration $config, $transactionId) {
		...
		if(!empty($orderSchema)) {
			...
			if ($totalLength > 30 ) {
				...
				$orderSchema = Customweb_Barclaycard_Util::substrUtf8($orderSchema, min($lengthToReduce, strlen($orderSchema)), strlen($orderSchema));
			}
		...
		return Customweb_Barclaycard_Util::substrUtf8($id, 0, 30);
	}
	
	public static function applyOrderDescriptionSchema(Customweb_Barclaycard_Configuration $config, $transactionId) {
	...
	public static function sendRequest($url, $parameters) {
		...
		if($handler->getStatusCode() != 200) {
			throw new Exception('Barclaycard Server response is: '
	...
	public static function calculateHash($parameters, $transaction, Customweb_Barclaycard_Configuration $configuration) {
