...
class Customweb_Barclaycard_BackendOperation_Adapter_CapturingParameterBuilder extends Customweb_Barclaycard_MaintenanceParameterBuilder {
	...
	protected function getOperationParameter() {
		if (Customweb_Payment_Util::amountEqual($this->getTransaction()->getAuthorizationAmount(), $this->captureAmount) || $this->close) {
			return array('OPERATION' => Customweb_Barclaycard_IAdapter::OPERATION_CAPTURE_FULL);
		}
		else {
			return array('OPERATION' => Customweb_Barclaycard_IAdapter::OPERATION_CAPTURE_PARTIAL);
		}
	}
