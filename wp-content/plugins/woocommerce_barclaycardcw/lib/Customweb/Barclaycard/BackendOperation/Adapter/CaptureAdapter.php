...
require_once 'Customweb/Barclaycard/BackendOperation/Adapter/CapturingParameterBuilder.php';
 ...
 *
 */
class Customweb_Barclaycard_BackendOperation_Adapter_CaptureAdapter extends Customweb_Barclaycard_AbstractAdapter 
	...
	public function partialCapture(Customweb_Payment_Authorization_ITransaction $transaction, $items, $close){
		...
		$builder = new Customweb_Barclaycard_BackendOperation_Adapter_CapturingParameterBuilder($transaction, $this->getContainer(), $amount, $close);
		...
		$transaction->partialCaptureByLineItems($items, $close, $additionalMessage);
		return true;
	}
