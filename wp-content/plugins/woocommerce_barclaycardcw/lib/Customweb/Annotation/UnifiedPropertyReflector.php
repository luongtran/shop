...
class Customweb_Annotation_UnifiedPropertyReflector implements Customweb_Annotation_IAnnotationReflector{
	...
	public function __construct($className, $name) {
		...
		if ($classReflector->hasMethod('get' . ucfirst($name))) {
			$this->getMethodReflector = new Customweb_Annotation_ReflectionAnnotatedMethod($className, 'get' . ucfirst($name));
		}

		if ($classReflector->hasMethod('set' . ucfirst($name))) {
			$this->setMethodReflector = new Customweb_Annotation_ReflectionAnnotatedMethod($className, 'set' . ucfirst($name));
		}
