...
BarclaycardCwUtil::includeClass('BarclaycardCw_RecurringOrderContext');

library_load_class_by_name('Customweb_Payment_Authorization_Recurring_ITransactionContext');

class BarclaycardCw_RecurringTransactionContext extends BarclaycardCw_TransactionContext implements Customweb_Payment_Authorization_Recurring_ITransactionContext
{
	protected $initialTransactionId;
	
	private $initialTransaction;
	
	public function __construct(BarclaycardCw_Transaction $transaction, $order, $paymentMethod, $amountToCharge, $productId) {
		parent::__construct($transaction, $order, $paymentMethod);
		$initialTransaction = BarclaycardCwUtil::getInitialTransactionByOrderId($order->id);
		...
		$this->orderContext = new BarclaycardCw_RecurringOrderContext($order, $paymentMethod, $amountToCharge, $productId);
	}
	...
	public function getInitialTransaction() {
		if ($this->initialTransaction === NULL) {
			$this->initialTransaction = BarclaycardCwUtil::getTransactionById($this->initialTransactionId);
		}
